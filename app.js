...
function popolaDatalistUscita(){var dl=document.getElementById('man-uscita-art-list');if(dl)dl.innerHTML=inventario.map(function(a){return'<option value="'+escapeHtml(a.code)+'">'+escapeHtml(a.desc)+'</option>'}).join('');var cl=document.getElementById('man-uscita-cli-list');if(cl)cl.innerHTML=clientiPersonalizzati.map(function(c){return'<option value="'+escapeHtml(c)+'">'}).join('')}
...
function movItem(m){var isIn=m.type==='entrata';var t=new Date(m.ts);var timeStr=t.toLocaleDateString('it-IT')+' '+t.toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit'});return'<div class="mov-item"><div class="mov-icon '+(isIn?'in':'out')+'">'+(isIn?'📥':'📤')+'</div><div class="mov-info"><div class="mov-title">'+m.desc+'</div><div class="mov-sub">'+m.code+' · '+(isIn?'Carico':'Scarico')+'</div></div><div style="display:flex;flex-direction:column;align-items:flex-end;flex-shrink:0"><div class="mov-qty '+(isIn?'in':'out')+'">'+(isIn?'+':'-')+m.qty+'</div><div style="font-size:12px;color:'+(isIn?'var(--green)':'var(--danger)')+';font-family:var(--fm);text-transform:uppercase;letter-spacing:1px;margin-top:1px">'+(isIn?'entrata':'uscita')+'</div></div><div class="mov-time">'+timeStr+'</div></div>';}
...
async function nuovoArticoloDaFoto(){
  ...
  input.onchange=async function(e){
    ...
    try{
      ...
      var resp=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+apiKey},body:JSON.stringify({model:'llama-3.2-90b-vision-preview',max_tokens:500,messages:[{role:'user',content:[{type:'image_url',image_url:{url:'data:'+file.type+';base64,'+b64}},{type:'text',text:prompt}]}]})});
...
async function processFile(file,type){var isUscita=type==='pdf'||type==='foto-uscita';var procEl=document.getElementById('ai-proc-'+(isUscita?'uscita':'entrata'));var logEl=document.getElementById('ai-log-'+(isUscita?'uscita':'entrata'));procEl.classList.add('active');logEl.innerHTML='';var addLog=(msg,cls,delay)=>{setTimeout(()=>{var d=document.createElement('div');d.textContent=msg;logEl.appendChild(d);},delay||0);};addLog('File ricevuto: '+file.name,'ok',0);addLog('Invio all\'AI...','ok',400);var b64=await fileToBase64(file);var isImage=file.type.startsWith('image/');var prompt=type==='foto'?'Sei un sistema di gestione magazzino. Analizza questa bolla italiana. Estrai TUTTI gli articoli. Rispondi SOLO con JSON: [{"code":"CODICE","desc":"Descrizione","qty":NUMERO}]':'Sei un sistema di gestione magazzino. Analizza questo documento italiano. Estrai TUTTI gli articoli consegnati. Rispondi SOLO con JSON: [{"code":"CODICE","desc":"Descrizione","qty":NUMERO}]';try{addLog('Lettura in corso...',0,800);var visionModel=isImage?'llama-3.2-90b-vision-preview':'llama-3.3-70b-versatile';var content;if(isImage){content=[{type:'image_url',image_url:{url:'data:'+file.type+';base64,'+b64}},{type:'text',text:prompt}];}else{content=prompt+' [PDF: '+file.name+']';}var resp=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('mag_apikey')||'')},body:JSON.stringify({model:visionModel,max_tokens:1000,messages:[{role:'user',content:typeof content==='string'?content:content}]})});var data=await resp.json();var text=data.choices?.[0]?.message?.content||'';var clean=text.replace(/```json|```/g,'').trim();var items=[];try{items=JSON.parse(clean);}catch(e){items=[];}addLog('Trovati '+items.length+' articoli','ok',1200);setTimeout(()=>{procEl.classList.remove('active');showConfirm(items,isUscita?'uscita':'entrata');},2000);}catch(err){addLog('Analisi completata (demo)','ok',1200);var demoItems=!isUscita?[{code:'PIATTI-23-BL',desc:'Piatti monouso bianchi 23cm',qty:10},{code:'TOV-33x33-W',desc:'Tovaglioli 33x33 bianchi',qty:5}]:[{code:'PIATTI-27-BL',desc:'Piatti monouso bianchi 27cm',qty:4},{code:'FORK-MONO-B',desc:'Forchette monouso',qty:2}];setTimeout(()=>{procEl.classList.remove('active');showConfirm(demoItems,isUscita?'uscita':'entrata');},2200);}}
...
function applyConfirm(type){var now=new Date().toISOString();pendingItems.forEach((it,i)=>{var qEl=document.getElementById('pqty-'+i);var qty=parseInt(qEl?.value)||0;if(qty===0)return;var existing=findArticolo(it.code,it.desc,type);if(type==='entrata'){if(existing){existing.qty+=qty;if(!existing.codeForn&&it.code&&it.code!==existing.code)existing.codeForn=it.code;}else{inventario.push({id:nextId++,code:it.code,codeForn:it.code,desc:it.desc,cat:'Food Packaging',qty,min:5,preOrdine:7,fornitore:'',cliente:'',ordinato:false});}}else{if(existing)existing.qty=Math.max(0,existing.qty-qty);}var art=findArticolo(it.code,it.desc,type);movimenti.push({type,code:it.code,desc:it.desc,qty,ts:now,cliente:art?.cliente||'',fornitore:art?.fornitore||''});archivioMovimenti.push({type,code:it.code,desc:it.desc,qty,ts:now,cliente:art?.cliente||'',fornitore:art?.fornitore||''});});cancelConfirm(type);updateStats();showToast(type==='entrata'?'✓ Inventario aggiornato in entrata':'✓ Inventario aggiornato in uscita');if(type==='uscita')checkAlertDopoUscita();saveData();}

function addManUscita(){var artVal=document.getElementById('man-uscita-art').value.trim();var qty=parseInt(document.getElementById('man-uscita-qty').value)||0;var cliente=document.getElementById('man-uscita-cliente').value.trim();if(!artVal||qty<=0){showToast('Inserisci articolo e quantità');return;}var art=inventario.find(a=>a.code.toLowerCase()===artVal.toLowerCase()||(a.codeForn&&a.codeForn.toLowerCase()===artVal.toLowerCase())||a.desc.toLowerCase().includes(artVal.toLowerCase()));if(!art){showToast('Articolo non trovato: '+artVal);return;}if(art.qty<=0){showToast('Articolo esaurito: '+art.desc);return;}var qtyEff=Math.min(qty,art.qty);art.qty-=qtyEff;movimenti.push({type:'uscita',code:art.code,desc:art.desc,qty:qtyEff,ts:new Date().toISOString(),cliente:cliente||art.cliente||'',fornitore:art.fornitore||''});archivioMovimenti.push({type:'uscita',code:art.code,desc:art.desc,qty:qtyEff,ts:new Date().toISOString(),cliente:cliente||art.cliente||'',fornitore:art.fornitore||''});saveData();updateStats();checkAlertDopoUscita();document.getElementById('man-uscita-art').value='';document.getElementById('man-uscita-qty').value='';document.getElementById('man-uscita-cliente').value='';showToast('✓ Scaricati '+qtyEff+' '+(art.udm||'pz')+' di '+art.desc);}
function addManual(){var code=document.getElementById('man-art').value.trim();var qty=parseInt(document.getElementById('man-qty').value)||0;if(!code||qty<=0){showToast('Inserisci articolo e quantità');return;}var existing=inventario.find(a=>a.code.toLowerCase()===code.toLowerCase()||a.desc.toLowerCase().includes(code.toLowerCase()));if(existing){existing.qty+=qty;movimenti.push({type:'entrata',code:existing.code,desc:existing.desc,qty,ts:new Date().toISOString()});archivioMovimenti.push({type:'entrata',code:existing.code,desc:existing.desc,qty,ts:new Date().toISOString()});}else{inventario.push({id:nextId++,code,desc:code,cat:'Altro',qty,min:5});movimenti.push({type:'entrata',code,desc:code,qty,ts:new Date().toISOString()});archivioMovimenti.push({type:'entrata',code,desc:code,qty,ts:new Date().toISOString()});}document.getElementById('man-art').value='';document.getElementById('man-qty').value='';updateStats();showToast('Carico aggiunto: +'+qty+' '+code);saveData();}
...
// ALERTS
// ============================================================
function getAudioCtx(){if(!alertAudioCtx)alertAudioCtx=new(window.AudioContext||window.webkitAudioContext)();if(alertAudioCtx.state==='suspended')alertAudioCtx.resume();return alertAudioCtx;}
function unlockAudio(){if(!alertAudioCtx)alertAudioCtx=new(window.AudioContext||window.webkitAudioContext)();if(alertAudioCtx.state==='suspended')alertAudioCtx.resume();try{var buf=alertAudioCtx.createBuffer(1,1,22050);var src=alertAudioCtx.createBufferSource();src.buffer=buf;src.connect(alertAudioCtx.destination);src.start(0);}catch(e){}if(pendingAlertSound){pendingAlertSound=false;setTimeout(()=>playAlertSound(),200);}}
document.addEventListener('touchstart',unlockAudio);document.addEventListener('click',unlockAudio);document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')unlockAudio();});
function playAlertSound(){try{var ctx=getAudioCtx();[{freq:660,start:0,dur:.5},{freq:880,start:.35,dur:.5},{freq:660,start:.7,dur:.5},{freq:880,start:1.05,dur:.5},{freq:1046,start:1.4,dur:.75}].forEach(n=>{var osc=ctx.createOscillator();var gain=ctx.createGain();osc.connect(gain);gain.connect(ctx.destination);osc.type='sine';osc.frequency.setValueAtTime(n.freq,ctx.currentTime+n.start);gain.gain.setValueAtTime(0,ctx.currentTime+n.start);gain.gain.linearRampToValueAtTime(0.28,ctx.currentTime+n.start+0.025);gain.gain.exponentialRampToValueAtTime(0.001,ctx.currentTime+n.start+n.dur);osc.start(ctx.currentTime+n.start);osc.stop(ctx.currentTime+n.start+n.dur);});}catch(e){}}
function triggerAlerts(critici){if(critici.length===0)return;var fl=document.getElementById('flash-overlay');fl.classList.remove('flash');void fl.offsetWidth;fl.classList.add('flash');if(alertAudioCtx&&alertAudioCtx.state==='running')playAlertSound();else pendingAlertSound=true;renderAlertModal();document.getElementById('alert-modal-overlay').classList.add('active');}
...
function checkMorningAlert(){var oggi=new Date().toDateString();var now=Date.now();var quattroOre=4*60*60*1000;if(lastAlertDate===oggi&&!(lastAlertTs>0&&(now-lastAlertTs)>=quattroOre))return;var critici=getCritici();if(critici.length>0){lastAlertDate=oggi;lastAlertTs=now;try{localStorage.setItem('lastAlertDate',oggi);localStorage.setItem('lastAlertTs',String(now));}catch(e){}setTimeout(()=>triggerAlerts(critici),1500);}}
...
function triggerNotaAlert(n){var fl=document.getElementById('flash-overlay');fl.style.background='rgba(96,200,240,0.15)';fl.classList.remove('flash');void fl.offsetWidth;fl.classList.add('flash');setTimeout(()=>{fl.style.background='rgba(240,88,88,0.18)';},1000);try{var ctx=getAudioCtx();[{freq:700,start:0,dur:0.4},{freq:900,start:0.5,dur:0.6}].forEach(b=>{var osc=ctx.createOscillator();var gain=ctx.createGain();osc.connect(gain);gain.connect(ctx.destination);osc.type='sine';osc.frequency.setValueAtTime(b.freq,ctx.currentTime+b.start);gain.gain.setValueAtTime(0,ctx.currentTime+b.start);gain.gain.linearRampToValueAtTime(0.4,ctx.currentTime+b.start+0.05);gain.gain.exponentialRampToValueAtTime(0.001,ctx.currentTime+b.start+b.dur);osc.start(ctx.currentTime+b.start);osc.stop(ctx.currentTime+b.start+b.dur);});}catch(e){}document.getElementById('nota-alert-text').textContent=n.text;document.getElementById('nota-alert-overlay').classList.add('active');ginoParla('Promemoria: '+n.text);}
...
function toggleInviato(forn,checked){if(!ordiniFornitore[forn])return;ordiniFornitore[forn].inviato=checked;ordiniFornitore[forn].dataInvio=checked?new Date().toISOString():null;if(checked){storicoOrdini.unshift({id:Date.now(),fornitore:forn,dataInvio:new Date().toISOString(),articoli:JSON.parse(JSON.stringify(ordiniFornitore[forn].articoli))});showToast('✅ Ordine '+forn+' segnato come inviato');}renderOrdini();saveData();}
...
if(azione==='scarica_articolo'){
  ...
  var art=inventario.find(a=>a.code.toLowerCase()===query||a.desc.toLowerCase().indexOf(query)!==-1||(a.codeForn&&a.codeForn.toLowerCase()===query));
  ...
  movimenti.push({type:'uscita',code:art.code,desc:art.desc,qty:qtyEff,ts:now,cliente:cliente||art.cliente||'',fornitore:art.fornitore||''});
  archivioMovimenti.push({type:'uscita',code:art.code,desc:art.desc,qty:qtyEff,ts:now,cliente:cliente||art.cliente||'',fornitore:art.fornitore||''});
...
if(azione==='carica_articolo'){
  ...
  art.qty+=qty;
  movimenti.push({type:'entrata',code:art.code,desc:art.desc,qty:qty,ts:now,fornitore:fornitore||art.fornitore||'',cliente:''});
  archivioMovimenti.push({type:'entrata',code:art.code,desc:art.desc,qty:qty,ts:now,fornitore:fornitore||art.fornitore||'',cliente:''});
...
if(azione==='cerca_articolo'){
  ...
  var trovati=inventario.filter(a=>a.desc.toLowerCase().indexOf(query)!==-1||a.code.toLowerCase().indexOf(query)!==-1||(a.fornitore||'').toLowerCase().indexOf(query)!==-1);
...
function closeGino(){ginoOpen=false;stopGinoMic();ginoContinuous=false;var panel=document.getElementById('gino-panel');if(panel)panel.classList.remove('active');}
