...
function movItem(m){var isIn=m.type==='entrata';var t=new Date(m.ts);var timeStr=t.toLocaleDateString('it-IT')+' '+t.toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit'});var unitLabel=m.unitType==='pezzi'?'pezzi':(m.udm||'');var unitBadge=m.unitType?'<span style="font-size:10px;padding:1px 6px;border-radius:4px;margin-left:4px;background:'+(m.unitType==='pezzi'?'rgba(255,140,66,.15);color:#ff8c42':'rgba(79,140,255,.15);color:#4f8cff')+'">'+(m.unitType==='pezzi'?'PEZZI':'CARTONI')+'</span>':'';var sfusiNote=m.pezziSfusi>0?' <span style="font-size:11px;color:#ff8c42">+ '+m.pezziSfusi+' pz sfusi</span>':'';return'<div class="mov-item"><div class="mov-icon '+(isIn?'in':'out')+'">'+(isIn?'📥':'📤')+'</div><div class="mov-info"><div class="mov-title">'+m.desc+'</div><div class="mov-sub">'+m.code+' · '+(isIn?'Carico':'Scarico')+unitBadge+'</div></div><div style="display:flex;flex-direction:column;align-items:flex-end;flex-shrink:0"><div class="mov-qty '+(isIn?'in':'out')+'">'+(isIn?'+':'-')+m.qty+' <span style="font-size:13px">'+unitLabel+'</span></div>'+sfusiNote+'<div style="font-size:12px;color:'+(isIn?'var(--green)':'var(--danger)')+';font-family:var(--fm);text-transform:uppercase;letter-spacing:1px;margin-top:1px">'+(isIn?'entrata':'uscita')+'</div></div><div class="mov-time">'+timeStr+'</div></div>';}
...
function aggiornaLabelPezzi(){var udm=document.getElementById('new-udm')?.value||'cartoni';var row=document.getElementById('pezzi-cartone-row');var label=document.getElementById('pezzi-cartone-label');var sfusiRow=document.getElementById('pezzi-sfusi-row');if(!row||!label)return;if(udm==='pezzi'){row.style.display='none';if(sfusiRow)sfusiRow.style.display='none';return;}row.style.display='block';var singolare={cartoni:'cartone',coppie:'coppia',confezioni:'confezione'};label.textContent='Pezzi per '+(singolare[udm]||udm)+' (opzionale)';var ppc=parseInt(document.getElementById('new-pezzi-cartone')?.value)||0;if(sfusiRow)sfusiRow.style.display=ppc>0?'block':'none';}
var _entrataUnit='cartoni';
var _uscitaUnit='cartoni';
function setEntrataUnit(unit){_entrataUnit=unit;var lbl=document.getElementById('man-qty-label');if(lbl)lbl.textContent=unit==='cartoni'?'Quantità (cartoni)':'Quantità (pezzi)';var sfusiRow=document.getElementById('man-sfusi-row');if(sfusiRow)sfusiRow.style.display=unit==='cartoni'?'block':'none';}
function setUscitaUnit(unit){_uscitaUnit=unit;var lbl=document.getElementById('man-usc-qty-label');if(lbl)lbl.textContent=unit==='cartoni'?'Quantità (cartoni)':'Quantità (pezzi)';}
...
function closeModal(){document.getElementById('modal-overlay').classList.remove('active');document.getElementById('modal-overlay').dataset.editId='';['new-code','new-code-forn','new-desc','new-qty','new-min','new-fornitore','new-cliente','new-preordine','new-corsia','new-pezzi-sfusi','new-pezzi-cartone','new-note-art','new-conf-per-unita','new-rotoli-per-unita','new-kg-per-unita'].forEach(id=>{var el=document.getElementById(id);if(el)el.value='';});_corsiaPhotoData=null;var preview=document.getElementById('corsia-photo-preview');if(preview)preview.style.display='none';}
...
function addManUscita(){var artVal=document.getElementById('man-uscita-art').value.trim();var descVal=document.getElementById('man-uscita-desc')?.value.trim()||'';var qty=parseInt(document.getElementById('man-uscita-qty').value)||0;var cliente=document.getElementById('man-uscita-cliente').value.trim();var query=artVal||descVal;if(!query||qty<=0){showToast('Inserisci articolo e quantità');return;}var art=cercaArticolo(query);if(!art){showToast('Articolo non trovato: '+artVal);return;}var unitType=_uscitaUnit;if(unitType==='pezzi'&&art.pezziPerCartone>0){var totPz=getTotalPezzi(art);if(totPz<=0){showToast('Articolo esaurito: '+art.desc);return;}var qtyEff=scaricaPezzi(art,qty);movimenti.push({type:'uscita',code:art.code,desc:art.desc,qty:qtyEff,ts:new Date().toISOString(),cliente:cliente||art.cliente||'',fornitore:art.fornitore||'',udm:art.udm||'cartoni',unitType:'pezzi'});archivioMovimenti.push({type:'uscita',code:art.code,desc:art.desc,qty:qtyEff,ts:new Date().toISOString(),cliente:cliente||art.cliente||'',fornitore:art.fornitore||'',udm:art.udm||'cartoni',unitType:'pezzi'});saveData();updateStats();checkAlertDopoUscita();showToast('✓ Scaricati '+qtyEff+' pezzi di '+art.desc);}else{if(art.qty<=0){showToast('Articolo esaurito: '+art.desc);return;}var qtyEff=Math.min(qty,art.qty);art.qty-=qtyEff;movimenti.push({type:'uscita',code:art.code,desc:art.desc,qty:qtyEff,ts:new Date().toISOString(),cliente:cliente||art.cliente||'',fornitore:art.fornitore||'',udm:art.udm||'cartoni',unitType:'cartoni'});archivioMovimenti.push({type:'uscita',code:art.code,desc:art.desc,qty:qtyEff,ts:new Date().toISOString(),cliente:cliente||art.cliente||'',fornitore:art.fornitore||'',udm:art.udm||'cartoni',unitType:'cartoni'});saveData();updateStats();checkAlertDopoUscita();showToast('✓ Scaricati '+qtyEff+' '+(art.udm||'cartoni')+' di '+art.desc);}document.getElementById('man-uscita-art').value='';document.getElementById('man-uscita-qty').value='';document.getElementById('man-uscita-cliente').value='';if(document.getElementById('man-uscita-desc'))document.getElementById('man-uscita-desc').value='';}
function addManual(){var code=document.getElementById('man-art').value.trim();var desc=document.getElementById('man-art-desc')?.value.trim()||'';var qty=parseInt(document.getElementById('man-qty').value)||0;var sfusi=parseInt(document.getElementById('man-sfusi')?.value)||0;var query=code||desc;if(!query||qty<=0){showToast('Inserisci articolo e quantità');return;}var existing=cercaArticolo(query);var unitType=_entrataUnit;if(existing){if(unitType==='pezzi'&&existing.pezziPerCartone>0){caricaPezzi(existing,qty);movimenti.push({type:'entrata',code:existing.code,desc:existing.desc,qty:qty,ts:new Date().toISOString(),udm:existing.udm||'cartoni',unitType:'pezzi'});archivioMovimenti.push({type:'entrata',code:existing.code,desc:existing.desc,qty:qty,ts:new Date().toISOString(),udm:existing.udm||'cartoni',unitType:'pezzi'});}else{existing.qty+=qty;if(sfusi>0&&existing.pezziPerCartone>0)caricaPezzi(existing,sfusi);movimenti.push({type:'entrata',code:existing.code,desc:existing.desc,qty:qty,pezziSfusi:sfusi,ts:new Date().toISOString(),udm:existing.udm||'cartoni',unitType:'cartoni'});archivioMovimenti.push({type:'entrata',code:existing.code,desc:existing.desc,qty:qty,pezziSfusi:sfusi,ts:new Date().toISOString(),udm:existing.udm||'cartoni',unitType:'cartoni'});}}else{inventario.push({id:nextId++,code,desc:code,cat:'Altro',qty:unitType==='pezzi'?0:qty,min:5,pezziSfusi:unitType==='pezzi'?qty:sfusi});movimenti.push({type:'entrata',code,desc:code,qty,ts:new Date().toISOString(),unitType:unitType});archivioMovimenti.push({type:'entrata',code,desc:code,qty,ts:new Date().toISOString(),unitType:unitType});}document.getElementById('man-art').value='';document.getElementById('man-qty').value='';if(document.getElementById('man-art-desc'))document.getElementById('man-art-desc').value='';if(document.getElementById('man-sfusi'))document.getElementById('man-sfusi').value='';updateStats();var label=unitType==='pezzi'?qty+' pezzi':qty+' cartoni'+(sfusi>0?' + '+sfusi+' pz sfusi':'');showToast('Carico aggiunto: +'+label+' '+(code||desc));saveData();}
...
// === GESTIONE PEZZI SFUSI ===
function getTotalPezzi(art){
  if(!art.pezziPerCartone||art.pezziPerCartone<=0)return null;
  return art.qty*art.pezziPerCartone+(art.pezziSfusi||0);
}
function getStockLabel(art){
  var tp=getTotalPezzi(art);
  if(tp===null)return art.qty+' '+(art.udm||'cartoni');
  return tp+' pz ('+art.qty+' ct'+(art.pezziSfusi>0?' + '+(art.pezziSfusi)+' sfusi':'')+')';
}
function scaricaPezzi(art,numPezzi){
  var sfusi=art.pezziSfusi||0;
  var totale=getTotalPezzi(art);
  if(totale===null||!art.pezziPerCartone)return 0;
  var eff=Math.min(numPezzi,totale);
  if(eff<=sfusi){art.pezziSfusi=sfusi-eff;}
  else{var rimanenti=eff-sfusi;art.pezziSfusi=0;var cartoniNecessari=Math.ceil(rimanenti/art.pezziPerCartone);art.qty-=cartoniNecessari;art.pezziSfusi=(cartoniNecessari*art.pezziPerCartone)-rimanenti;}
  return eff;
}
function caricaPezzi(art,numPezzi){
  var sfusi=(art.pezziSfusi||0)+numPezzi;
  if(art.pezziPerCartone&&art.pezziPerCartone>0){var cartoniInteri=Math.floor(sfusi/art.pezziPerCartone);art.qty+=cartoniInteri;art.pezziSfusi=sfusi-(cartoniInteri*art.pezziPerCartone);}
  else{art.pezziSfusi=sfusi;}
}
...
inventario.map(function(a){
  var pzInfo=a.pezziPerCartone>0?' | pz/ct:'+a.pezziPerCartone+' | sfusi:'+(a.pezziSfusi||0)+' | tot_pz:'+getTotalPezzi(a):'';
  return a.code+' | '+a.desc+' | qty:'+a.qty+' '+(a.udm||'ct')+pzInfo+' | min:'+a.min+' | forn:'+( a.fornitore||'-')+( a.cliente?' | cliente:'+a.cliente:'')+(a.qty<=a.min?' ⚠️':'');
}).join('\n');}
...
if(azione==='scarica_articolo'){
  var query=(params.articolo||'').toLowerCase();var qty=parseInt(params.qty)||0;var cliente=params.cliente||'';var unitType=params.unitType||'';
  ...
  if(qty<=0)return'Quanti '+(unitType||getUdm(art))+' di '+art.desc+' devo scaricare?';
  if(!unitType&&art.pezziPerCartone>0)return'Intendi '+qty+' cartoni o '+qty+' pezzi di '+art.desc+'?';
  if(unitType==='pezzi'&&art.pezziPerCartone>0){
    var totPz=getTotalPezzi(art);
    if(totPz<=0)return art.desc+' è esaurito, zero pezzi in magazzino.';
    var qtyEff=scaricaPezzi(art,qty);
    movimenti.push({type:'uscita',code:art.code,desc:art.desc,qty:qtyEff,ts:now,cliente:cliente||art.cliente||'',fornitore:art.fornitore||'',udm:art.udm||'cartoni',unitType:'pezzi'});
    archivioMovimenti.push({type:'uscita',code:art.code,desc:art.desc,qty:qtyEff,ts:now,cliente:cliente||art.cliente||'',fornitore:art.fornitore||'',udm:art.udm||'cartoni',unitType:'pezzi'});
    saveData();updateStats();checkAlertDopoUscita();refreshViste();
    var msg='Scaricati '+qtyEff+' pezzi di '+art.desc+'.';
    var newTot=getTotalPezzi(art);msg+=' Rimangono '+art.qty+' cartoni e '+(art.pezziSfusi||0)+' sfusi ('+newTot+' pz totali).';
    if(art.qty<=art.min)msg+=' Attenzione, scorta bassa!';
    return msg;
  }else{
    if(art.qty<=0)return art.desc+' è esaurito, zero in magazzino.';
    var qtyEff=Math.min(qty,art.qty);art.qty-=qtyEff;
    movimenti.push({type:'uscita',code:art.code,desc:art.desc,qty:qtyEff,ts:now,cliente:cliente||art.cliente||'',fornitore:art.fornitore||'',udm:art.udm||'cartoni',unitType:'cartoni'});
    archivioMovimenti.push({type:'uscita',code:art.code,desc:art.desc,qty:qtyEff,ts:now,cliente:cliente||art.cliente||'',fornitore:art.fornitore||'',udm:art.udm||'cartoni',unitType:'cartoni'});
    saveData();updateStats();checkAlertDopoUscita();refreshViste();
...
if(azione==='carica_articolo'){
  var query=(params.articolo||'').toLowerCase();var qty=parseInt(params.qty)||0;var fornitore=params.fornitore||'';var unitType=params.unitType||'';
  if(!query)return'Quale articolo devo caricare?';
  var art=cercaArticolo(query);
  if(!art)return'Non trovo "'+params.articolo+'" in magazzino.';
  if(qty<=0)return'Quanti '+(unitType||getUdm(art))+' di '+art.desc+'?';
  if(!unitType&&art.pezziPerCartone>0)return'Intendi '+qty+' cartoni o '+qty+' pezzi di '+art.desc+'?';
  if(unitType==='pezzi'&&art.pezziPerCartone>0){
    caricaPezzi(art,qty);
    movimenti.push({type:'entrata',code:art.code,desc:art.desc,qty:qty,ts:now,fornitore:fornitore||art.fornitore||'',cliente:'',udm:art.udm||'cartoni',unitType:'pezzi'});
    archivioMovimenti.push({type:'entrata',code:art.code,desc:art.desc,qty:qty,ts:now,fornitore:fornitore||art.fornitore||'',cliente:'',udm:art.udm||'cartoni',unitType:'pezzi'});
    saveData();updateStats();refreshViste();
    return'Caricati '+qty+' pezzi di '+art.desc+'. Totale ora: '+art.qty+' cartoni e '+(art.pezziSfusi||0)+' sfusi ('+getTotalPezzi(art)+' pz).';
  }else{
    art.qty+=qty;
    movimenti.push({type:'entrata',code:art.code,desc:art.desc,qty:qty,ts:now,fornitore:fornitore||art.fornitore||'',cliente:'',udm:art.udm||'cartoni',unitType:'cartoni'});
    archivioMovimenti.push({type:'entrata',code:art.code,desc:art.desc,qty:qty,ts:now,fornitore:fornitore||art.fornitore||'',cliente:'',udm:art.udm||'cartoni',unitType:'cartoni'});
    saveData();updateStats();refreshViste();
...
(function(){if(archivioMovimenti.length>2000)archivioMovimenti=archivioMovimenti.slice(-2000);})();
renderDashboard();
